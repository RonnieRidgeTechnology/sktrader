...
import { FolderTree, Plus, Pencil, Trash2, Check, X, GripVertical, Search, FolderOpen } from 'lucide-vue-next';
...
const dragged = ref(null); // { id, parent_id }
...
function applySearch() {
  ...
  router.get(route('admin.categories.index'), params, { preserveState: true });
}

const total = computed(() => props.categories?.length ?? 0);

function destroy(id, name) {
  if (confirm(`Delete category "${name}"? This may affect products.`)) {
...
<template>
  <AdminLayout>
    <div class="space-y-6">
      <PageHeader title="Categories" description="Product categories for organizing your catalog. Drag rows to change order.">
        <template #actions>
          <Link
      ...
      <DataCard class="p-4">
        <div class="flex flex-nowrap items-end gap-4">
          <div class="min-w-0 flex-1">
            <label for="cat-search" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Search</label>
      ...
      <DataCard class="p-0">
        <template v-if="categories?.length">
          ...
          <!-- Table: top-level categories only -->
            ...
            <table class="admin-table min-w-full">
              <thead>
                <tr>
                  ...
                  <th scope="col" class="admin-th">Sub-categories</th>
              ...
              <tbody class="divide-y divide-zinc-200/80 dark:divide-zinc-700/80">
                ...
                >
                  ...
                  <td class="admin-td">
                    <Link
                      :href="route('admin.categories.subcategories', cat.id)"

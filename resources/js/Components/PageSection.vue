...
const props = defineProps({
  theme: { type: String, default: 'luxe' }, // luxe = dark cinematic (default)
...
const mode = computed(() => props.theme || 'luxe'); // luxe | editorial | classic
const isEditorial = computed(() => mode.value === 'editorial');
const isLuxe = computed(() => mode.value === 'luxe');
...
<template>
  ...
  >
    <!-- Background pattern -->
    <div
      v-if="backgroundPattern === 'steps' && isClassic"
    ...
    <div class="relative mx-auto min-w-0 max-w-7xl" :class="containerClass">
      <template v-if="$slots.vector">
        <div class="grid gap-8 lg:grid-cols-12 lg:gap-16 lg:items-stretch">
          <div class="order-2 flex min-w-0 justify-center lg:order-1 lg:col-span-5 lg:min-h-0 xl:col-span-4">
            <div
              class="w-full max-w-[260px] sm:max-w-[280px] lg:max-w-none lg:min-h-[280px] lg:h-full aspect-square lg:aspect-auto"
              :class="isLuxe ? 'text-luxe-mist/60' : (isEditorial ? 'text-editorial-slate' : 'text-zinc-400 dark:text-zinc-500')"
            >
              <slot name="vector" />
            </div>
          </div>
          <div class="order-1 min-w-0 lg:order-2 lg:col-span-7 xl:col-span-8">
            <h2
              v-if="title"
              class="font-bold tracking-tight sm:text-3xl lg:text-4xl"
              :class="isLuxe ? 'font-display text-3xl text-luxe-pearl' : (isEditorial ? 'font-editorial text-2xl text-editorial-ink' : 'text-2xl text-zinc-900 dark:text-white')"
            >
              {{ title }}
            </h2>
            <p
              v-if="subtitle"
              class="mt-3 max-w-2xl text-base sm:text-lg"
              :class="isLuxe ? 'text-luxe-pearl/80' : (isEditorial ? 'text-editorial-slate' : 'text-zinc-600 dark:text-zinc-400')"
            >
              {{ subtitle }}
            </p>
            <div class="mt-8">
      ...
      <template v-else>
        <h2
          ...
          :class="isLuxe ? 'font-display text-center text-luxe-pearl' : (isEditorial ? 'font-editorial text-center text-editorial-ink' : 'text-center text-zinc-900 dark:text-white')"
        >
          {{ title }}
        </h2>
        <p
          v-if="subtitle"
          class="mx-auto mt-3 max-w-2xl text-center text-base sm:text-lg"
          :class="isLuxe ? 'text-luxe-pearl/80' : (isEditorial ? 'text-editorial-slate' : 'text-zinc-600 dark:text-zinc-400')"
        >
          {{ subtitle }}
        </p>

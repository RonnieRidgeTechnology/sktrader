...
class HomepageSectionController extends Controller
{
    ...
    private function defaultsForSectionKey(string $key): array
    {
        $defaults = [
            ...
            'reels_strip' => ['title' => 'Video reels', 'visible' => true, 'sort_order' => 50, 'data' => ['eyebrow' => 'Showroom & style', 'heading' => 'Videos are managed in Admin → Video Reels.']],
    ...
    public function update(Request $request, HomepageSection $section)
    {
        if ($section->key === 'hero') {
            ...
            $data = $section->data ?? [];
            $data['headline'] = $request->input('headline') ?: 'Quality Sofas & Furniture for Every Home';
            $data['subheading'] = $request->input('subheading') ?: 'Comfort, Style & Value in Zambia';
